...
const GetSMS = () => {
    ...
    const handleSubmitSMS = e => {
        ...
        getShipment(shipment)
            .then((response) => {
                if (response.data.sms !== null) toast.info(`SMS-код сообщение от отправления: ${response.data.sms}`)
                else toast.warning("Вы не выслали SMS-код, попробуйте еще раз")
            })
            .catch((err) => {
