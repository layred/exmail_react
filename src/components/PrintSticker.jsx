...
import { getPochtaShipment, getShipment, getSticker } from '../api/shipments'
...
const PrintSticker = () => {
    ...
    const parseSticker = (shipmentData) => {
        getSticker(shipmentData.id).then((response) => {
    ...
    const handleSubmit = e => {
        ...
        getShipment(shipmentid).then((response) => {
            parseSticker(response.data)
        }).catch((err) => {
            if (err.response.status === 404) {
                getPochtaShipment(shipmentid).then((response) => {
                    if (response.data === {}) toast.warning("Отправление не найдено!")
                    else {
                        getShipment(response.data).then((response) => {
                            parseSticker(response.data)
                        }).catch((err) => toast.warning("Произошла ошибка, попробуйте позже"))
                    }
                })
            }
            else toast.error("Произошла ошибка, попробуйте позже")
        })
        setIsLoading(false);
    }
    ...
    return (
        <>
            <div className='rounded-lg w-auto h-auto bg-gray-100 p-8 flex flex-col justify-center shadow-md'>
                ...
                <iframe src="" type="application/pdf" title="Shipment PDF" width="100%" height="100%" style={{"overflow": "auto"}}></iframe>
            </div>
        </>
    )
}
